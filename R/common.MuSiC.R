...
deconv.MuSiC <- function(bulk,feature,selCellType=NULL){
  SC <- feature$SC
  rm(feature)
  SC <- Biobase::ExpressionSet(as.matrix(SC),
                      phenoData=Biobase::AnnotatedDataFrame(data.frame(row.names=colnames(SC),
                                                              cellType=base::sapply(strsplit(colnames(SC),"\\|"),head,1),
                                                              dID=base::sapply(strsplit(colnames(SC),"\\|"),"[[",2))))
  if(!is.null(selCellType)) SC <- SC[,base::sapply(strsplit(sampleNames(SC),"\\|"),head,1)%in%selCellType]
  comp <- music_prop(Biobase::ExpressionSet(as.matrix(bulk)),SC,clusters='cellType',samples='dID')
...
music_prop <- function (bulk.eset, sc.eset, markers = NULL, clusters, samples, 
                        ...
                        normalize = FALSE, ...) 
{
  bulk.gene = rownames(bulk.eset)[rowMeans(Biobase::exprs(bulk.eset)) != 
                                    0]
  ...
  Yjg = relative.ab(Biobase::exprs(bulk.eset)[m.bulk, ])
                         ...
                         select.ct = NULL, cell_size = NULL, ct.cov = FALSE, verbose = TRUE) 
{
  if (!is.null(select.ct)) {
    s.ct = sampleNames(x)[as.character(xbioc::pVar(x, clusters)) %in% 
                            select.ct]
    x <- x[, s.ct, drop = FALSE]
  }
  if (non.zero) {
  ...
  if (ct.cov) {
    ...
    Theta <- sapply(unique(clusters), function(ct) {
      sapply(unique(samples), function(sid) {
        y = Biobase::exprs(x)[, clusters %in% ct & samples %in% 
                       sid, drop = FALSE]
